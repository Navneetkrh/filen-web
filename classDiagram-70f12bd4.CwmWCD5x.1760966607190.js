import { s as A, d as S, p as G } from "./styles-9a916d00.Brq39RTM.1760966607190.js";
import { g as v, l as y, d as B, e as W, r as $, u as M, X as I } from "./index.Dfumm3oK.1760966607190.js";
import { G as X } from "./graph.DhjqBz52.1760966607190.js";
import { l as O } from "./layout.CqBSxywX.1760966607190.js";
import "./percentages-BXMCSKIN.D0yBlsLW.1760966607190.js";
import { l as P } from "./line.jIL4itrg.1760966607190.js";
...
const Y = function(i, a, t, o, p) {
  ...
  let u, m, w, k;
  if (x % 2 !== 0 && x > 1) {
    let e = M.calcCardinalityPosition(t.relation.type1 !== "none", a.points, a.points[0]), d = M.calcCardinalityPosition(t.relation.type2 !== "none", a.points, a.points[x - 1]);
  ...
  const p = a.id, g = { id: p, label: a.id, width: 0, height: 0 }, s = i.append("g").attr("id", o.db.lookUpDomId(p)).attr("class", "classGroup");
  let c;
  ...
  a.annotations.forEach(function(d) {
    const l = c.append("tspan").text("\xAB" + d + "\xBB");
  ...
  const p = a.id, g = { id: p, text: a.text, width: 0, height: 0 }, s = i.append("g").attr("id", p).attr("class", "classGroup");
...
`);
  ...
  const r = s.node().getBBox(), h = s.insert("rect", ":first-child").attr("x", 0).attr("y", 0).attr("width", r.width + 2 * t.padding).attr("height", r.height + n.length * t.textHeight + t.padding + 0.5 * t.dividerMargin).node().getBBox().width;
...
const E = 20, L = function(i) {
  ...
  R(n);
  const r = new X({ multigraph: true });
  r.setGraph({ isMultiGraph: true }), r.setDefaultEdgeLabel(function() {
    return {};
  });
  ...
  o.db.getRelations().forEach(function(e) {
    y.info("tjoho" + L(e.id1) + L(e.id2) + JSON.stringify(e)), r.setEdge(L(e.id1), L(e.id2), { relation: e }, e.title || "DEFAULT");
  }), o.db.getNotes().forEach(function(e) {
    y.debug(`Adding note: ${JSON.stringify(e)}`);
    const d = N.drawNote(n, e, p, o);
    T[d.id] = d, r.setNode(d.id, d), e.class && e.class in f && r.setEdge(e.id, L(e.class), { relation: { id1: e.id, id2: e.class, relation: { type1: "none", type2: "none", lineType: 10 } } }, "DEFAULT");
  }), O(r), r.nodes().forEach(function(e) {
    e !== void 0 && r.node(e) !== void 0 && (y.debug("Node " + e + ": " + JSON.stringify(r.node(e))), c.select("#" + (o.db.lookUpDomId(e) || e)).attr("transform", "translate(" + (r.node(e).x - r.node(e).width / 2) + "," + (r.node(e).y - r.node(e).height / 2) + " )"));
...
}, U = { draw: F }, at = { parser: G, db: S, renderer: U, styles: A, init: (i) => {
