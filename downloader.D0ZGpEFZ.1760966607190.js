import { c as w, e as b } from "./index.1760966607190.js";
const { WritableStream: y, TransformStream: m, DOMException: P, Blob: _ } = w, { GONE: v } = b, E = /constructor/i.test(window.HTMLElement);
class A {
  ...
  async createWritable(e = {}) {
    var _a;
    const t = await ((_a = navigator.serviceWorker) == null ? void 0 : _a.getRegistration()), o = document.createElement("a"), n = new m(), u = n.writable;
    if (o.download = this.name, E || !t) {
      let r = [];
      n.readable.pipeTo(new y({ write(s) {
        r.push(new _([s]));
      }, close() {
      ...
      const { writable: r, readablePort: s } = new S(y), a = encodeURIComponent(this.name).replace(/['()]/g, escape).replace(/\*/g, "%2A"), R = { "content-disposition": "attachment; filename*=UTF-8''" + a, "content-type": "application/octet-stream; charset=utf-8", ...e.size ? { "content-length": e.size } : {} }, f = setTimeout(() => t.active.postMessage(0), 1e4);
      n.readable.pipeThrough(new m({ transform(c, d) {
        ...
        clearInterval(f);
      }), t.active.postMessage({ url: t.scope + a, headers: R, readablePort: s }, [s]);
      const i = document.createElement("iframe");
      i.hidden = true, i.src = t.scope + a, document.body.appendChild(i);
    }
    return u.getWriter();
  }
}
...
class k {
  ...
  _resolveReady() {
    this._readyResolve(), this._readyPending = false;
  }
  _rejectReady(e) {
    this._readyPending || this._resetReady(), this._readyPromise.catch(() => {
    }), this._readyReject(e), this._readyPending = false;
  }
}
class S {
  constructor(e) {
