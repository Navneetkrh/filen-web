import { G as M } from "./graph.DhjqBz52.1760966607190.js";
import { a8 as R, ab as F, ac as j, a6 as U, l as h, g as S, d as $, u as H, Y as K, _ as A, $ as E, a0 as L, V as G, j as z, t as W } from "./index.Dfumm3oK.1760966607190.js";
import { r as X } from "./index-3862675e.DqXX75S3.1760966607190.js";
import { c as Y } from "./channel.BNwECGkz.1760966607190.js";
function J(e) {
  return new R([document.querySelectorAll(e)], [document.documentElement]);
}
function pe(e, l) {
...
const V = {}, ee = function(e) {
  const l = Object.keys(e);
  for (const c of l) V[c] = e[c];
}, P = async function(e, l, c, a, i, d) {
  ...
  for (const p of n) {
    ...
    if (h.info("vertex", r, r.labelType), r.labelType === "markdown") h.info("vertex", r, r.labelType);
    else if (W(S().flowchart.htmlLabels)) s = O(u, { label: t }).node(), s.parentNode.removeChild(s);
    else {
...
`), S()), n.style === void 0 && (t.style = t.style || "stroke: #333; stroke-width: 1.5px;fill:none;"), t.labelStyle = t.labelStyle.replace("color:", "fill:"), t.id = r, t.classes = "flowchart-link " + y + " " + w, l.setEdge(n.start, n.end, t, a);
  ...
  let r;
  d === "sandbox" && (r = $("#i" + l));
  const y = d === "sandbox" ? $(r.nodes()[0].contentDocument.body) : $("body"), w = d === "sandbox" ? r.nodes()[0].contentDocument : document, t = new M({ multigraph: true, compound: true }).setGraph({ rankdir: i, nodesep: n, ranksep: p, marginx: 0, marginy: 0 }).setDefaultEdgeLabel(function() {
    return {};
  });
  let s;
  const b = a.db.getSubGraphs();
  h.info("Subgraphs - ", b);
  for (let f = b.length - 1; f >= 0; f--) s = b[f], h.info("Subgraph - ", s), a.db.addVertex(s.id, { text: s.title, type: s.labelType }, "group", void 0, s.classes, s.dir);
  const o = a.db.getVertices(), T = a.db.getEdges();
  h.info("Edges", T);
  let k = 0;
  for (k = b.length - 1; k >= 0; k--) {
    s = b[k], J("cluster").append("text");
    for (let f = 0; f < s.nodes.length; f++) h.info("Setting up subgraphs", s.nodes[f], s.id), t.setParent(s.nodes[f], s.id);
  }
  await P(o, t, l, y, w, a), await q(T, t);
  const _ = y.select(`[id="${l}"]`), C = y.select("#" + l + " g");
  if (await X(C, t, ["point", "circle", "cross"], "flowchart", l), H.insertTitle(_, "flowchartTitleText", u.titleTopMargin, a.db.getDiagramTitle()), K(t, _, u.diagramPadding, u.useMaxWidth), a.db.indexNodes("subGraph" + k), !u.htmlLabels) {
...
}, we = { setConf: ee, addVertices: P, addEdges: q, getClasses: te, draw: re }, le = (e, l) => {
