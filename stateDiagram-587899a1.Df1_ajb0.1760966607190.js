import { s as P, d as N, p as W } from "./styles-6aaf32cf.BcpWhUxF.1760966607190.js";
import { g as t, d as H, l as b, e as R, j as T, r as v, u as U } from "./index.Dfumm3oK.1760966607190.js";
import { G as C } from "./graph.DhjqBz52.1760966607190.js";
import { l as F } from "./layout.CqBSxywX.1760966607190.js";
import "./percentages-BXMCSKIN.D0yBlsLW.1760966607190.js";
import { l as $ } from "./line.jIL4itrg.1760966607190.js";
...
const O = (e) => e.append("circle").attr("class", "start-state").attr("r", t().state.sizeUnit).attr("cx", t().state.padding + t().state.sizeUnit).attr("cy", t().state.padding + t().state.sizeUnit), X = (e) => e.append("line").style("stroke", "grey").style("stroke-dasharray", "3").attr("x1", t().state.textHeight).attr("class", "divider").attr("x2", t().state.textHeight * 2).attr("y1", 0).attr("y2", 0), J = (e, i) => {
...
}, _ = (e) => (e.append("circle").attr("class", "end-state-outer").attr("r", t().state.sizeUnit + t().state.miniPadding).attr("cx", t().state.padding + t().state.sizeUnit + t().state.miniPadding).attr("cy", t().state.padding + t().state.sizeUnit + t().state.miniPadding), e.append("circle").attr("class", "end-state-inner").attr("r", t().state.sizeUnit).attr("cx", t().state.padding + t().state.sizeUnit + 2).attr("cy", t().state.padding + t().state.sizeUnit + 2)), q = (e, i) => {
  ...
  const o = i.id, c = { id: o, label: i.id, width: 0, height: 0 }, s = e.append("g").attr("id", o).attr("class", "stateGroup");
...
const K = function(e, i, o) {
  ...
  let a = "";
  if (t().state.arrowMarkerAbsolute && (a = window.location.protocol + "//" + window.location.host + window.location.pathname + window.location.search, a = a.replace(/\(/g, "\\("), a = a.replace(/\)/g, "\\)")), p.attr("marker-end", "url(" + a + "#" + c(N.relationType.DEPENDENCY) + "End)"), o.title !== void 0) {
...
const z = {}, Q = function() {
  ...
  A(y, r, void 0, false, p, a, c);
  const x = B.padding, d = r.node().getBBox(), l = d.width + x * 2, m = d.height + x * 2, w = l * 1.75;
  R(r, m, w, B.useMaxWidth), r.attr("viewBox", `${d.x - B.padding}  ${d.y - B.padding} ` + l + " " + m);
  ...
  o ? a.setGraph({ rankdir: "LR", multigraph: true, compound: true, ranker: "tight-tree", ranksep: y ? 1 : B.edgeLengthFactor, nodeSep: y ? 1 : 50, isMultiGraph: true }) : a.setGraph({ rankdir: "TB", multigraph: true, compound: true, ranksep: y ? 1 : B.edgeLengthFactor, nodeSep: y ? 1 : 50, ranker: "tight-tree", isMultiGraph: true }), a.setDefaultEdgeLabel(function() {
    return {};
  }), p.db.extract(e);
  ...
  for (const n of l) {
    ...
    if (u.note) {
      const f = { descriptions: [], id: u.id + "-note", note: u.note, type: "note" }, S = L(i, f);
  ...
  d.forEach(function(n) {
    m++, b.debug("Setting edge", n), a.setEdge(n.id1, n.id2, { relation: n, width: tt(n.title), height: B.labelHeight * T.getRows(n.title).length, labelpos: "c" }, "id" + m);
  }), F(a), b.debug("Graph after layout", a.nodes());
  const w = i.node();
  a.nodes().forEach(function(n) {
    n !== void 0 && a.node(n) !== void 0 ? (b.warn("Node " + n + ": " + JSON.stringify(a.node(n))), s.select("#" + w.id + " #" + n).attr("transform", "translate(" + (a.node(n).x - a.node(n).width / 2) + "," + (a.node(n).y + (z[n] ? z[n].y : 0) - a.node(n).height / 2) + " )"), s.select("#" + w.id + " #" + n).attr("data-x-shift", a.node(n).x - a.node(n).width / 2), g.querySelectorAll("#" + w.id + " #" + n + " .divider").forEach((h) => {
  ...
  const k = { id: o || "root", label: o || "root", width: 0, height: 0 };
  return k.width = E.width + 2 * B.padding, k.height = E.height + 2 * B.padding, b.debug("Doc rendered", k, a), k;
}, et = { setConf: Q, draw: D }, gt = { parser: W, db: N, renderer: et, styles: P, init: (e) => {
