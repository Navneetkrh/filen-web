...
function E(e) {
  this._ += e[0];
  for (let t = 1, h = e.length; t < h; ++t) this._ += arguments[t] + e[t];
}
function A(e) {
  let t = Math.floor(e);
  ...
  return function(i) {
    this._ += i[0];
    for (let s = 1, n = i.length; s < n; ++s) this._ += Math.round(arguments[s] * h) / h + i[s];
  };
}
class L {
  constructor(t) {
  ...
  arcTo(t, h, i, s, n) {
    ...
    else if (a > u) if (!(Math.abs(o * p - l * _) > u) || !n) this._append`L${this._x1 = t},${this._y1 = h}`;
    else {
