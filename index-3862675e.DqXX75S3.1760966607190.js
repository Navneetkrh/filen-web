import { i as N, G as A } from "./graph.DhjqBz52.1760966607190.js";
import { m as $, l as H } from "./layout.CqBSxywX.1760966607190.js";
import { c as V } from "./clone.BL4jgHi2.1760966607190.js";
import { i as U, u as W, s as _, c as q, b as z, g as D, d as O, a as K, p as Q, e as p, f as J, h as Y, j as Z } from "./edges-e0da2a9e.pUEwyU0X.1760966607190.js";
import { l as s, g as T, t as S, d as L } from "./index.Dfumm3oK.1760966607190.js";
import { c as I } from "./createText-2e5e7dd3.BtlU1DrF.1760966607190.js";
function m(e) {
...
const nt = () => {
  g = {}, R = {}, l = {};
}, B = (e, t) => (s.trace("In isDescendant", t, " ", e, " = ", g[t].includes(e)), !!g[t].includes(e)), it = (e, t) => (s.info("Descendants of ", t, " is ", g[t]), s.info("Edge is ", e), e.v === t || e.w === t ? false : g[t] ? g[t].includes(e.v) || B(e.v, t) || B(e.w, t) || g[t].includes(e.w) : (s.debug("Tilt, ", t, ",not in descendants"), false)), P = (e, t, n, r) => {
  ...
  e !== r && i.push(e), s.warn("Copying (nodes) clusterId", e, "nodes", i), i.forEach((a) => {
    if (t.children(a).length > 0) P(a, t, n, r);
      ...
      const d = t.node(a);
      s.info("cp ", a, " to ", r, " with parent ", e), n.setNode(a, d), r !== t.parent(a) && (s.warn("Setting parent", a, t.parent(a)), n.setParent(a, t.parent(a))), e !== r && a !== e ? (s.debug("Setting parent", a, e), n.setParent(a, e)) : (s.info("In copy ", e, "root", r, "data", t.node(e), r), s.debug("Not Setting parent for node=", a, "cluster!==rootId", e !== r, "node!==clusterId", a !== e));
      const u = t.edges(a);
      s.debug("Copying Edges", u), u.forEach((f) => {
        ...
        try {
          it(f, r) ? (s.info("Copying as ", f.v, f.w, h, f.name), n.setEdge(f.v, f.w, h, f.name), s.info("newGraph edges ", n.edges(), n.edge(n.edges()[0]))) : s.info("Skipping copy of edge ", f.v, "-->", f.w, " rootId: ", r, " clusterId:", e);
        } catch (w) {
          s.error(w);
        }
      });
  ...
  e.nodes().forEach(function(n) {
    e.children(n).length > 0 && (s.warn("Cluster identified", n, " Replacement id in edges: ", C(n, e)), g[n] = k(n, e), l[n] = { id: C(n, e), clusterData: e.node(n) });
  }), e.nodes().forEach(function(n) {
    const r = e.children(n), i = e.edges();
    r.length > 0 ? (s.debug("Cluster identified", n, g), i.forEach((a) => {
      if (a.v !== n && a.w !== n) {
        const d = B(a.v, n), u = B(a.w, n);
        d ^ u && (s.warn("Edge: ", a, " leaves cluster ", n), s.warn("Descendants of XXX ", n, ": ", g[n]), l[n].externalConnections = true);
      }
  ...
  s.debug("Nodes = ", n, t);
  for (const i of n) if (s.debug("Extracting node", i, l, l[i] && !l[i].externalConnections, !e.parent(i), e.node(i), e.children("D"), " Depth ", t), !l[i]) s.debug("Not a cluster", i, t);
    ...
    const u = new A({ multigraph: true, compound: true }).setGraph({ rankdir: d, nodesep: 50, ranksep: 50, marginx: 8, marginy: 8 }).setDefaultEdgeLabel(function() {
      return {};
    });
    s.warn("Old graph before copy", m(e)), P(i, e, u, i), e.setNode(i, { clusterNode: true, id: i, clusterData: l[i].clusterData, labelText: l[i].labelText, graph: u }), s.warn("New graph after copy node: (", i, ")", m(u)), s.debug("Old graph after copy", m(e));
  } else s.warn("Cluster ** ", i, " **not meeting the criteria !externalConnections:", !l[i].externalConnections, " no parent: ", !e.parent(i), " children ", e.children(i) && e.children(i).length > 0, e.children("D"), t), s.debug(l);
  ...
  const n = T(), r = e.insert("g").attr("class", "cluster" + (t.class ? " " + t.class : "")).attr("id", t.id), i = r.insert("rect", ":first-child"), a = S(n.flowchart.htmlLabels), d = r.insert("g").attr("class", "cluster-label"), u = t.labelType === "markdown" ? I(d, t.labelText, { style: t.labelStyle, useHtmlLabels: a }) : d.node().appendChild(J(t.labelText, t.labelStyle, void 0, true));
  let f = u.getBBox();
  ...
  a ? d.attr("transform", `translate(${t.x - f.width / 2}, ${t.y - t.height / 2 + v})`) : d.attr("transform", `translate(${t.x}, ${t.y - t.height / 2 + v})`);
  ...
  const n = T(), r = e.insert("g").attr("class", t.classes).attr("id", t.id), i = r.insert("rect", ":first-child"), a = r.insert("g").attr("class", "cluster-label"), d = r.append("rect"), u = a.node().appendChild(J(t.labelText, t.labelStyle, void 0, true));
  let f = u.getBBox();
  if (S(n.flowchart.htmlLabels)) {
    const c = u.children[0], o = L(u);
    f = c.getBoundingClientRect(), o.attr("width", f.width), o.attr("height", f.height);
...
const dt = (e, t) => {
  ...
  await Promise.all(t.nodes().map(async function(c) {
    const o = t.node(c);
    if (i !== void 0) {
    ...
    if (s.info("(Insert) Node XXX" + c + ": " + JSON.stringify(t.node(c))), o && o.clusterNode) {
      ...
      const b = await M(x, o.graph, n, r, t.node(c), a), E = b.elem;
  ...
  return rt(t).forEach(function(c) {
    const o = t.node(c);
    s.info("Position " + c + ": " + JSON.stringify(t.node(c))), s.info("Position " + c + ": (" + o.x, "," + o.y, ") width: ", o.width, " height: ", o.height), o && o.clusterNode ? (o.y += y, O(o)) : t.children(c).length > 0 ? (o.height += y, dt(f, o), l[o.id].node = o) : (o.y += y / 2, O(o));
  }), t.edges().forEach(function(c) {
    const o = t.edge(c);
