import { l as g, r as lt, g as b, t as H, d as E, u as j, i as q, v as Q } from "./index.Dfumm3oK.1760966607190.js";
import { c as st } from "./createText-2e5e7dd3.BtlU1DrF.1760966607190.js";
import { l as ct } from "./line.jIL4itrg.1760966607190.js";
const ht = (e, t, a, i) => {
  t.forEach((l) => {
    wt[l](e, a, i);
  });
...
}, wt = { extension: ot, composition: yt, aggregation: pt, dependency: ft, lollipop: xt, point: dt, circle: gt, cross: ut, barb: bt }, hr = ht;
function mt(e, t) {
  t && e.attr("style", t);
}
function kt(e) {
...
const vt = (e, t, a, i) => {
  ...
  if (typeof l == "object" && (l = l[0]), H(b().flowchart.htmlLabels)) {
    ...
    const r = { isNode: i, label: Q(l).replace(/fa[blrs]?:fa-[\w-]+/g, (n) => `<i class='${n.replace(":", " ")}'></i>`), labelStyle: t.replace("fill:", "color:") };
    return kt(r);
  } else {
  ...
  const o = n.node();
  let h;
  t.labelType === "markdown" ? h = st(n, q(Q(c), b()), { useHtmlLabels: r, width: t.width || b().flowchart.wrappingWidth, classes: "markdown-node-label" }) : h = o.appendChild(R(q(Q(c), b()), t.labelStyle, false, i));
  let y = h.getBBox();
  const f = t.padding / 2;
  if (H(b().flowchart.htmlLabels)) {
    ...
    if (k) {
      ...
      await Promise.all([...k].map((u) => new Promise((S) => {
        function B() {
...
function Mt(e, t, a, i) {
  ...
  if (l = t.y - e.y, s = e.x - t.x, c = t.x * e.y - e.x * t.y, f = l * a.x + s * a.y + c, p = l * i.x + s * i.y + c, !(f !== 0 && p !== 0 && J(f, p)) && (r = i.y - a.y, n = a.x - i.x, o = i.x * a.y - a.x * i.y, h = r * e.x + n * e.y + o, y = r * t.x + n * t.y + o, !(h !== 0 && y !== 0 && J(h, y)) && (d = l * n - r * s, d !== 0))) return k = Math.abs(d / 2), x = s * o - n * c, u = x < 0 ? (x - k) / d : (x + k) / d, x = r * c - l * o, S = x < 0 ? (x - k) / d : (x + k) / d, { x: u, y: S };
}
function J(e, t) {
  return e * t > 0;
}
function Tt(e, t, a) {
  ...
  for (var c = i - e.width / 2 - s, o = l - e.height / 2 - n, h = 0; h < t.length; h++) {
    var y = t[h], f = t[h < t.length - 1 ? h + 1 : 0], p = Mt(e, a, { x: c + y.x, y: o + y.y }, { x: c + f.x, y: o + f.y });
    p && r.push(p);
  }
...
const Bt = (e, t) => {
...
}, Et = Bt, w = { node: Lt, circle: St, ellipse: it, polygon: Tt, rect: Et }, Ct = async (e, t) => {
  t.useHtmlLabels || b().flowchart.htmlLabels || (t.centerLabel = true);
  const { shapeSvg: i, bbox: l, halfPadding: r } = await M(e, t, "node " + t.classes, true);
...
}, K = (e) => e ? " " + e : "", _ = (e, t) => `node default${K(e.classes)} ${K(e.class)}`, P = async (e, t) => {
  const { shapeSvg: a, bbox: i } = await M(e, t, _(t), true), l = i.width + t.padding, r = i.height + t.padding, s = l + r, n = [{ x: s / 2, y: 0 }, { x: s, y: -s / 2 }, { x: s / 2, y: -s }, { x: 0, y: -s / 2 }];
  ...
  const a = e.insert("g").attr("class", "node default").attr("id", t.domId || t.id), i = 28, l = [{ x: 0, y: i / 2 }, { x: i / 2, y: 0 }, { x: 0, y: -28 / 2 }, { x: -28 / 2, y: 0 }];
  ...
  const { shapeSvg: a, bbox: i } = await M(e, t, _(t), true), l = 4, r = i.height + t.padding, s = r / l, n = i.width + 2 * s + t.padding, c = [{ x: s, y: 0 }, { x: n - s, y: 0 }, { x: n, y: -r / 2 }, { x: n - s, y: -r }, { x: s, y: -r }, { x: 0, y: -r / 2 }], o = I(a, n, r, c);
  return o.attr("style", t.style), m(t, o), t.intersect = function(h) {
    return w.polygon(t, c, h);
  }, a;
}, Nt = async (e, t) => {
  const { shapeSvg: a, bbox: i } = await M(e, t, void 0, true), l = 2, r = i.height + 2 * t.padding, s = r / l, n = i.width + 2 * s + t.padding, c = Rt(t.directions, i, t), o = I(a, n, r, c);
  return o.attr("style", t.style), m(t, o), t.intersect = function(h) {
    return w.polygon(t, c, h);
  }, a;
}, Ot = async (e, t) => {
  const { shapeSvg: a, bbox: i } = await M(e, t, _(t), true), l = i.width + t.padding, r = i.height + t.padding, s = [{ x: -r / 2, y: 0 }, { x: l, y: 0 }, { x: l, y: -r }, { x: -r / 2, y: -r }, { x: 0, y: -r / 2 }];
  return I(a, l, r, s).attr("style", t.style), t.width = l + r, t.height = r, t.intersect = function(c) {
    return w.polygon(t, s, c);
  }, a;
}, Wt = async (e, t) => {
  const { shapeSvg: a, bbox: i } = await M(e, t, _(t), true), l = i.width + t.padding, r = i.height + t.padding, s = [{ x: -2 * r / 6, y: 0 }, { x: l - r / 6, y: 0 }, { x: l + 2 * r / 6, y: -r }, { x: r / 6, y: -r }], n = I(a, l, r, s);
  ...
  const { shapeSvg: a, bbox: i } = await M(e, t, _(t), true), l = i.width + t.padding, r = i.height + t.padding, s = [{ x: -2 * r / 6, y: 0 }, { x: l + 2 * r / 6, y: 0 }, { x: l - r / 6, y: -r }, { x: r / 6, y: -r }], n = I(a, l, r, s);
  return n.attr("style", t.style), m(t, n), t.intersect = function(c) {
    return w.polygon(t, s, c);
  }, a;
}, Dt = async (e, t) => {
  const { shapeSvg: a, bbox: i } = await M(e, t, _(t), true), l = i.width + t.padding, r = i.height + t.padding, s = [{ x: r / 6, y: 0 }, { x: l - r / 6, y: 0 }, { x: l + 2 * r / 6, y: -r }, { x: -2 * r / 6, y: -r }], n = I(a, l, r, s);
  return n.attr("style", t.style), m(t, n), t.intersect = function(c) {
    return w.polygon(t, s, c);
  }, a;
}, At = async (e, t) => {
  const { shapeSvg: a, bbox: i } = await M(e, t, _(t), true), l = i.width + t.padding, r = i.height + t.padding, s = [{ x: 0, y: 0 }, { x: l + r / 2, y: 0 }, { x: l, y: -r / 2 }, { x: l + r / 2, y: -r }, { x: 0, y: -r }], n = I(a, l, r, s);
  return n.attr("style", t.style), m(t, n), t.intersect = function(c) {
    return w.polygon(t, s, c);
  }, a;
}, jt = async (e, t) => {
  const { shapeSvg: a, bbox: i } = await M(e, t, _(t), true), l = i.width + t.padding, r = l / 2, s = r / (2.5 + l / 50), n = i.height + s + t.padding, c = "M 0," + s + " a " + r + "," + s + " 0,0,0 " + l + " 0 a " + r + "," + s + " 0,0,0 " + -l + " 0 l 0," + n + " a " + r + "," + s + " 0,0,0 " + l + " 0 l 0," + -n, o = a.attr("label-offset-y", s).insert("path", ":first-child").attr("style", t.style).attr("d", c).attr("transform", "translate(" + -l / 2 + "," + -(n / 2 + s) + ")");
  ...
  const { shapeSvg: a, bbox: i, halfPadding: l } = await M(e, t, "node " + t.classes + " " + t.class, true), r = a.insert("rect", ":first-child"), s = t.positioned ? t.width : i.width + t.padding, n = t.positioned ? t.height : i.height + t.padding, c = t.positioned ? -s / 2 : -i.width / 2 - l, o = t.positioned ? -n / 2 : -i.height / 2 - l;
  ...
  const { shapeSvg: a, bbox: i, halfPadding: l } = await M(e, t, "node " + t.classes, true), r = a.insert("rect", ":first-child"), s = t.positioned ? t.width : i.width + t.padding, n = t.positioned ? t.height : i.height + t.padding, c = t.positioned ? -s / 2 : -i.width / 2 - l, o = t.positioned ? -n / 2 : -i.height / 2 - l;
  ...
  const { shapeSvg: a } = await M(e, t, "label", true);
...
const Gt = (e, t) => {
  ...
  let f = o.getBBox();
  const p = s.node().appendChild(R(y.join ? y.join("<br/>") : y, t.labelStyle, true, true));
  if (H(b().flowchart.htmlLabels)) {
    const k = p.children[0], x = E(p);
    h = k.getBoundingClientRect(), x.attr("width", h.width), x.attr("height", h.height);
  }
  const d = t.padding / 2;
  return E(p).attr("transform", "translate( " + (h.width > f.width ? 0 : (f.width - h.width) / 2) + ", " + (f.height + d + 5) + ")"), E(o).attr("transform", "translate( " + (h.width < f.width ? 0 : -(f.width - h.width) / 2) + ", 0)"), h = s.node().getBBox(), s.attr("transform", "translate(" + -h.width / 2 + ", " + (-h.height / 2 - d + 3) + ")"), l.attr("class", "outer title-state").attr("x", -h.width / 2 - d).attr("y", -h.height / 2 - d).attr("width", h.width + t.padding).attr("height", h.height + t.padding), r.attr("class", "divider").attr("x1", -h.width / 2 - d).attr("x2", h.width / 2 + d).attr("y1", -h.height / 2 - d + f.height + d).attr("y2", -h.height / 2 - d + f.height + d), m(t, l), t.intersect = function(k) {
    return w.rect(t, k);
  }, i;
}, Ft = async (e, t) => {
  const { shapeSvg: a, bbox: i } = await M(e, t, _(t), true), l = i.height + t.padding, r = i.width + l / 4 + t.padding, s = a.insert("rect", ":first-child").attr("style", t.style).attr("rx", l / 2).attr("ry", l / 2).attr("x", -r / 2).attr("y", -l / 2).attr("width", r).attr("height", l);
  return m(t, s), t.intersect = function(n) {
    return w.rect(t, n);
  }, a;
}, Qt = async (e, t) => {
  const { shapeSvg: a, bbox: i, halfPadding: l } = await M(e, t, _(t), true), r = a.insert("circle", ":first-child");
  ...
  const { shapeSvg: a, bbox: i, halfPadding: l } = await M(e, t, _(t), true), r = 5, s = a.insert("g", ":first-child"), n = s.insert("circle"), c = s.insert("circle");
  ...
  const { shapeSvg: a, bbox: i } = await M(e, t, _(t), true), l = i.width + t.padding, r = i.height + t.padding, s = [{ x: 0, y: 0 }, { x: l, y: 0 }, { x: l, y: -r }, { x: 0, y: -r }, { x: 0, y: 0 }, { x: -8, y: 0 }, { x: l + 8, y: 0 }, { x: l + 8, y: -r }, { x: -8, y: -r }, { x: -8, y: 0 }], n = I(a, l, r, s);
  return n.attr("style", t.style), m(t, n), t.intersect = function(c) {
    return w.polygon(t, s, c);
  }, a;
}, Jt = (e, t) => {
  ...
  let p = 0;
  const d = t.classData.annotations && t.classData.annotations[0], k = t.classData.annotations[0] ? "\xAB" + t.classData.annotations[0] + "\xBB" : "", x = f.node().appendChild(R(k, t.labelStyle, true, true));
  let u = x.getBBox();
  if (H(b().flowchart.htmlLabels)) {
  ...
  const B = f.node().appendChild(R(S, t.labelStyle, true, true));
  ...
  t.classData.members.forEach((v) => {
    ...
    const N = f.node().appendChild(R(W, L.cssStyle ? L.cssStyle : t.labelStyle, true, true));
    let $ = N.getBBox();
    if (H(b().flowchart.htmlLabels)) {
  ...
  let nt = (h - C.width) / 2;
  return E(B).attr("transform", "translate( " + (-1 * h / 2 + nt) + ", " + (-1 * y / 2 + p) + ")"), p += C.height + i, c.attr("class", "divider").attr("x1", -h / 2 - a).attr("x2", h / 2 + a).attr("y1", -y / 2 - a + l + p).attr("y2", -y / 2 - a + l + p), p += l, X.forEach((v) => {
...
}, rt = { rhombus: P, composite: zt, question: P, rect: Ut, labelRect: Zt, rectWithTitle: Gt, choice: Ht, circle: Qt, doublecircle: Vt, stadium: Ft, hexagon: It, block_arrow: Nt, rect_left_inv_arrow: Ot, lean_right: Wt, lean_left: Xt, trapezoid: Yt, inv_trapezoid: Dt, rect_right_inv_arrow: At, cylinder: jt, start: Jt, end: Kt, note: $t, subroutine: qt, fork: tt, join: tt, class_box: Pt };
let Y = {};
...
const Z = (e) => Array.isArray(e) ? { x: e[0], y: e[1] } : e, rr = (e) => ({ x: function(t, a, i) {
  let l = 0;
...
}, er = { arrow_cross: "cross", arrow_point: "point", arrow_barb: "barb", arrow_circle: "circle", aggregation: "aggregation", extension: "extension", composition: "composition", dependency: "dependency", lollipop: "lollipop" }, at = (e, t, a, i, l, r) => {
  const s = er[a];
  if (!s) {
...
const xr = () => {
  ...
  const a = H(b().flowchart.htmlLabels), i = t.labelType === "markdown" ? st(e, t.label, { style: t.labelStyle, useHtmlLabels: a, addSvgBackground: true }) : R(t.label, t.labelStyle), l = e.insert("g").attr("class", "edgeLabel"), r = l.insert("g").attr("class", "label");
...
const gr = (e, t) => {
  ...
  return e.forEach((r) => {
    if (!sr(t, r) && !l) {
      ...
      a.forEach((c) => {
        n = n || c.x === s.x && c.y === s.y;
      }), a.some((c) => c.x === s.x && c.y === s.y) || a.push(s), l = true;
    } else i = r, l || a.push(r);
  }), a;
  ...
  const o = r.node(t.v);
  var h = r.node(t.w);
  (h == null ? void 0 : h.intersect) && (o == null ? void 0 : o.intersect) && (n = n.slice(1, a.points.length - 1), n.unshift(o.intersect(n[0])), n.push(h.intersect(n[n.length - 1]))), a.toCluster && (g.debug("to cluster abc88", i[a.toCluster]), n = et(a.points, i[a.toCluster].node), c = true), a.fromCluster && (g.debug("from cluster abc88", i[a.fromCluster]), n = et(n.reverse(), i[a.fromCluster].node).reverse(), c = true);
